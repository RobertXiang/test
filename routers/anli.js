...
//接口地址：http://127.0.0.1:3000/anli/anli_id?anli=1
//新加 整屋案例 通过id获取设计师信息
router.get("/anli_id", (req, res, next) => {
  let obj = req.query;
  let sql = "select * from yzal where anli_id=?";
  pool.query(sql, [obj.anli_id], (err, result) => {
    if (err) {
      next(err);
      return;
    }
