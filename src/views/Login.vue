<template>
  <div class="log">
    ...
    <!-- 背景图样式设置 -->
    <van-image width="100vw"  src="https://ikj-storage-front-prod.oss-cn-beijing.aliyuncs.com/app/html-pc/xsybj/xsybjhead.png" />
    <!-- 表单验证单 -->
      ...
      <van-tab title="账号密码登录" class="box">
        ...
        <van-form>
          <van-field
            ...
            placeholder="请输入用户名"
            left-icon="friends"
            :rules="[{ pattern: /^\w{3,15}$/ ,message: '请输入正确用户名'}]"
          />
          <van-field
            v-model="pwd"
            name="密码"
...
export default {
    ...
    methods:{
        ...
        onBlur(){
          this.axios.post('/user/login/uname',`uname=${this.name}&upwd=${this.pwd}`).then(result=>{
            ...
            if(result.data.code==200){
                    Toast({
                        ...
                        //修改vuex中的登录状态
                        this.$store.commit('loginok',{uname:this.name,uid:result.data.data.uid})
        ...
        onSubmit(){
            this.axios.post('/user/login/phone',`phone=${this.phone}&upwd=${this.pwd}`)
            .then((result)=>{
                console.log(result.data)
                if(result.data.code==200){
                    Toast({
                        ...
                        //修改vuex中的登录状态
                        this.$store.commit('loginok',{phone:this.phone,uid:result.data.data.uid})
                        //将islogin phone uid存入sessionStorage
                        sessionStorage.setItem('islogin',true)
